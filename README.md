...
* リポジトリの構築:
  ...
  **YEAR_LIMIT:** - 自動更新を行う最大年数（2021に設定した場合、2021/12/31まで実行）
...
* ワークフローの作成と実行:
  ...
  * リポジトリのActions > Import from ParaTranzでワークフローのページを開き、Run workflowを選ぶと手動でワークフローを実行することができます。  
...
* Discordへの通知:
  * 以下のSecretsを作成すると、GitHhub Pagesが更新されたときにDiscordへ通知メッセージが送られます。通知メッセージには翻訳された行数、ワード数といった統計情報も含まれます。  
  ...
  |DISCORD_DETAILS|TrueかFalse|（任意）Trueにすると変更があったファイルを一覧表示します。|
  |DISCORD_INTERVAL|整数|（任意）通知を行うまでのインターバルを1時間単位で設定できます。たとえば24にすると、前の通知から24時間経過するまで（それまでにActionが完走したとしても）通知しなくなります。|
